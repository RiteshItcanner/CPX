...
extension Keychain {
    ...
    //Get and Set Current Referral UserID. Set nil to delete.
    static var referralUserID: String? {
        get {
            return try? Keychain(service: bundleIdentifier, account: "referralUserID").readItem()
        }
        set {
            guard let value = newValue else {
                try? Keychain(service: bundleIdentifier, account: "referralUserID").deleteItem()
                return
            }
            do {
                try Keychain(service: bundleIdentifier, account: "referralUserID").saveItem(value)
            } catch {
                print("Unable to save Referral UserID to keychain.")
            }
        }
    }
    //Get and Set Current playerId. Set nil to delete.
    static var playerId: String? {
        get {
            return try? Keychain(service: bundleIdentifier, account: "playerId").readItem()
        }
        set {
            guard let value = newValue else {
                try? Keychain(service: bundleIdentifier, account: "playerId").deleteItem()
                return
            }
            do {
                try Keychain(service: bundleIdentifier, account: "playerId").saveItem(value)
            } catch {
                print("Unable to save playerId to keychain.")
            }
        }
    }
